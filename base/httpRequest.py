...
class HttpRequest(object):
    ...
    def send_post_request(self, base_url, payload, Headers):
        ...
        r.close()
        return str(status_code), response_json, response_time  # 返回響應碼，内容
    ...
    def sendRequest(self, method, path, data):
        base_url = self.config.showinpay_config() + path
        if method == "GET":
            response_data = self.send_get_request(base_url, data, showinpay_config.Get_showinpay_Headers)
        elif method == "POST":
            response_data = self.send_post_request(base_url, data, showinpay_config.Post_showinpay_Headers)
