...
class Showinpay_Http(HttpRequest_interface):  # Agent的http方法繼承(inherit) http介面
    def get(self, *args):
    ...
    def post_login(self, *args):
        r = requests.post(args[0], json = args[1], headers = args[2])
        status_code = r.status_code  # 獲取返回狀態碼
        response_json = r.content
        print(r.json())

        return str(status_code), response_json  # 返回響應碼，内容
